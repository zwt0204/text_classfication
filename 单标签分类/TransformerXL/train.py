...
class ModelTrainner:
    def __init__(self, is_training=True):
        ...
        with tf.variable_scope('classification_query'):
            self.model = Model_Classs(vocab_file="vocab.json", keep_prob=self.keep_prob, is_training=self.is_training)
        self.saver = tf.train.Saver()

    def train(self, epochs=20):
        xitems, yitems = self.load_samples_test(datafiles='train.txt')
        ...
        with tf.Session() as session:
            ...
            for epoch in range(epochs):
                ...
                for i in range(batch_count):
                    ...
                    batch_loss_value, _ = session.run([self.model.cost, self.model.optimizer], feed_dict)
    ...
    def read_sample_file(self, datafile):
        ...
        with open(datafile, "r+", encoding="utf-8") as reader:
            for line in reader:
                ...
                classid = record['label']
                raw_text = record['data']
