...
class Model_Class:
    def __init__(self, vocab_file, num_class=2, gpu_num=2, is_training=True):
    ...
    def create_model(self):
        with tf.name_scope("classification_cnn"):
            ...
            with tf.name_scope("fc"):
                W = tf.Variable(tf.truncated_normal(shape=[num_filter_total, self.fc_hidden_size],
            ...
            with tf.name_scope('dropout'):
                self.h_drop = tf.nn.dropout(self.fc_out, self.keep_pro)

            with tf.name_scope('output'):
                w = tf.get_variable("w", shape=[self.fc_hidden_size, self.num_classes],
                                    initializer=xavier_initializer())
                ...
                self.accuracy = tf.reduce_mean(tf.to_float(tf.equal(self.predictions, self.input_y)))
